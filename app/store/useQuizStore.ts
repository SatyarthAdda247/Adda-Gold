  ...
  persist(
    (set, get) => ({
      ...
      selectOption: ({ quizId, option, elapsedMs }) => {
        ...
        const isCorrect = quiz.correct === option;
        const answer: QuizProgress = {
          quizId,
          selected: option,
          isCorrect,
          elapsedMs,
        ...
        const newStats: FeedProgress = {
          answered: state.stats.answered + 1,
          correct: state.stats.correct + (isCorrect ? 1 : 0),
          streak: computeStreak(state.stats, isCorrect),
          totalTimeMs: state.stats.totalTimeMs + elapsedMs,
          lastSeenIndex: Math.max(state.currentIndex, state.stats.lastSeenIndex),
        };
        set({
